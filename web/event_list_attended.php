...
error_reporting(E_ALL);
  ...
  $sql = "SELECT eventName, DATE_FORMAT(eventDate, '%M %d %Y'), 
TIME_FORMAT(eventStartTime, '%r'), TIME_FORMAT(eventEndTime, '%r'),
userAttended, TIME_FORMAT(userCheckInTime, '%r')
  FROM event 
JOIN organization ON event.orgID = organization.orgID
JOIN userAccounts_has_event on event_eventID = event.eventID
			...
			echo "<div class='table-responsive'>";
				...
				if ($result->num_rows > 0) {
					...
					while($row = $result->fetch_assoc()) {
						...
						echo "<td>" . $row["TIME_FORMAT(userCheckInTime, '%r')"] . "</td>";
						echo "</tr>";			
					}
				} else {
					echo "0 results";
